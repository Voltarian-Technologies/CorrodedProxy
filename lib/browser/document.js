    ...
    return function rewriteDocument() {
            ...
            ctx.window.Element.prototype.setAttribute = new Proxy(ctx.window.Element.prototype.setAttribute, {
                apply: (target, that, args) => {
                    if (args[0] && args[1]) {
                        ...
                        switch(handler) {
                            case 'url':
                                Reflect.apply(target, that, [`corroded-${args[0]}`, args[1]]);
                            ...
                            case 'html':
                                ...
                                break;
                            case 'delete':
                                return Reflect.apply(target, that, [`corroded-${args[0]}`, args[1]]);
                        };
                    };
                    return Reflect.apply(target, that, args);
                },
        ...
        if (ctx.window.Element.prototype.getAttribute) {
            ctx.window.Element.prototype.getAttribute = new Proxy(ctx.window.Element.prototype.getAttribute, {
                apply: (target, that, args) => {
                    if (args[0] && that.hasAttribute(`corroded-${args[0]}`)) args[0] = `corroded-${args[0]}`;
                    return Reflect.apply(target, that, args);
                },
            });
        }; 
                            ...
                            apply: (target, that, args) => {
                                ...
                                switch(entry.handler) {
                                    ...
                                    case 'srcset':
                                        ...
                                        break;
                                    case 'delete':
                                        val = that.getAttribute(`corroded-${property}`);
                                        break;
                                    case 'window':
                                        try {
                                            if (!val.$corroded)  {
                                                val.$corroded = new ctx.constructor({ ...ctx.config, window: val, });
                                                val.$corroded.init();
                                                val.$corroded.meta = ctx.meta;
                                            };
                                        } catch(e) {};
                                };
                                return val;

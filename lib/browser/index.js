...
class Corroded extends require('../rewrite') {
    constructor(config = defaultConfig) {
        super(Object.assign(defaultConfig, config));
    const corroded = this;
    if (!this.config.window) throw 'Corroded Error: No window was given.';
        ...
        this.meta = {
            ...
            get base() {
                if (corroded.serviceWorker) return corroded._url;
                return corroded.window.document.baseURI != corroded.location.href ? new URL(corroded.window.document.baseURI) : corroded._url;
            },
            url: this._url,
        };
    ...
    get parent() {
        ...
        try {
            return this.window.parent.$corroded ? this.window.parent : this.window;
        } catch(e) {
            return this.window;
        };
    };
    get top() {
        if (this.serviceWorker) return false;
        try {
            return this.window.top.$corroded ? this.window.top : this.parent;
        } catch(e) {
            return this.parent;
        };
    };
    ...
    this.window.$corrodedGet$ = this.get$.bind(this);
    this.window.$corrodedSet$ = this.set$.bind(this);
    this.window.$corrodedGet$m = this.get$m.bind(this);
    this.window.$corrodedSet$m = this.set$m.bind(this);
    this.window.$corrodedCall$m = this.call$m.bind(this);
    };
    get$m(obj, key) {
        if (!this.serviceWorker && this.window != this.window.parent && obj == this.window.parent) {
            return this.parent.$corroded.get$m(this.parent, key);
        };
        if (!this.serviceWorker && this.window != this.window.top && obj == this.window.top) {
            return this.top.$corroded.get$m(this.top, key);
        };
    ...
    set$m(obj, key, val, operator) {
        if (!this.serviceWorker && this.window != this.window.parent && obj == this.window.parent) {
            return this.parent.$corroded.set$m(this.parent, key, val, operator);
        };
        if (!this.serviceWorker && this.window != this.window.top && obj == this.window.top) {
            return this.top.$corroded.set$m(this.top, key, val, operator);
        };
    ...
    call$m(obj, key, args) {
        if (!this.serviceWorker && this.window != this.window.parent && obj == this.window.parent) {
            return this.parent.$corroded.call$m(this.parent, key, args);
        };
        if (!this.serviceWorker && this.window != this.window.top && obj == this.window.top) {
            return this.top.$corroded.call$m(this.top, key, args);
        };
        return obj[key](...args);
    };
    get$(obj) {

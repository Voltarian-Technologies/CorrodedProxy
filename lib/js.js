...
class JSRewriter {
    ...
    createHead(url) {
        ...
        if (!self.$corrosion && self.importScripts) {
                ...
                self.$corroded = new Corroded({ url: '${url}', codec: '${this.ctx.config.codec || 'plain'}', serviceWorker: true,  window: self, prefix: '${this.ctx.prefix || '/service/'}', ws: ${this.ctx.config.ws || true}, cookies: ${this.ctx.config.cookies || false}, title: '${this.ctx.config.title}', }); $corroded.init();
        };\n`;
    };
    iterate(ast, handler) {
